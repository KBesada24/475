...
def draw_label(draw: ImageDraw.ImageDraw, xy: tuple[int, int], text: str, font: ImageFont.ImageFont) -> None:
    x, y = xy
    draw.rectangle((x - 6, y - 4, x + 438, y + 22), fill=(30, 37, 44))
    draw.text((x, y + 3), text, fill=(244, 238, 222), font=font)
...
def generated_scene(width: int = WIDTH, height: int = HEIGHT) -> np.ndarray:
...
def render_binary_text(text: str, size: tuple[int, int] = (420, 260)) -> np.ndarray:
...
def make_noisy_text() -> np.ndarray:
    base = render_binary_text("THE\nTEST\nIMAGE")
    rng = np.random.default_rng(475)
    noise = rng.random(base.shape) > 0.988
    scratches = np.zeros_like(base)
    h, w = base.shape
    for _ in range(55):
        x0 = int(rng.integers(0, w - 35))
        y0 = int(rng.integers(0, h - 8))
        length = int(rng.integers(14, 58))
        thickness = int(rng.integers(1, 3))
        scratches[y0 : min(h, y0 + thickness), x0 : min(w, x0 + length)] = True
    return base | noise | scratches


def make_broken_text() -> np.ndarray:
    base = render_binary_text("THE\nTEST\nIMAGE")
    rng = np.random.default_rng(8475)
    holes = rng.random(base.shape) > 0.976
    gaps = np.zeros_like(base)
    h, w = base.shape
    for _ in range(34):
        x0 = int(rng.integers(0, w - 26))
        y0 = int(rng.integers(0, h - 8))
        gaps[y0 : min(h, y0 + 3), x0 : min(w, x0 + 24)] = True
    return base & ~(holes | gaps)


def binary_to_rgb(mask: np.ndarray) -> np.ndarray:
    return gray_to_rgb(mask.astype(float))


def opening_closing_challenge() -> list[tuple[str, np.ndarray]]:
    noisy = make_noisy_text()
    opened = dilate(erode(noisy, 1), 1)

    broken = make_broken_text()
    closed = erode(dilate(broken, 3), 3)

    return [
        ("Noisy binary image", binary_to_rgb(noisy)),
        ("Opening: erode, then dilate", binary_to_rgb(opened)),
        ("Broken / holed binary image", binary_to_rgb(broken)),
        ("Closing: dilate, then erode", binary_to_rgb(closed)),
    ]


def rgb_to_hsv_saturation(image: np.ndarray) -> np.ndarray:
    ...
    return np.divide(chroma, max_channel, out=np.zeros_like(chroma), where=max_channel > 1e-8)


def channel_features(channel: np.ndarray) -> dict[str, float]:
...
def draw_histogram_panel(
    ...
    size: tuple[int, int],
    histograms: list[tuple[str, np.ndarray, tuple[int, int, int]]],
    features: dict[str, dict[str, float]],
    font: ImageFont.ImageFont,
) -> None:
    x0, y0 = origin
    width, height = size
    draw.rectangle((x0, y0, x0 + width, y0 + height), fill=(248, 246, 236), outline=(30, 37, 44), width=2)
    draw.text((x0 + 14, y0 + 12), "Feature vector summary", fill=(30, 37, 44), font=font)
    ...
    for name, hist, color in histograms:
        ...
        for idx, count in enumerate(hist):
            px = chart_x + int(idx * chart_w / (len(hist) - 1))
    ...
    for name in ("brightness", "saturation", "darkness"):
        stats = features[name]
        line = (
            f"{name:<12}  {stats['mean']:.3f}    {stats['std']:.3f}    "
...
def naturalness_features(source: np.ndarray) -> np.ndarray:
    brightness = np.max(source, axis=2)
    darkness = np.min(source, axis=2)
    difference = brightness - darkness
    saturation = rgb_to_hsv_saturation(source)
    ...
    features = {name: channel_features(channel) for name, channel in channels.items()}
    histograms = [
        ("brightness", np.histogram(brightness, bins=48, range=(0, 1))[0], (230, 90, 48)),
        ("saturation", np.histogram(saturation, bins=48, range=(0, 1))[0], (44, 126, 178)),
        ("darkness", np.histogram(darkness, bins=48, range=(0, 1))[0], (54, 82, 96)),
    ]

    tile_w, tile_h = 480, 320
    canvas = Image.new("RGB", (tile_w * 2, tile_h * 3), (248, 246, 236))
    font = ImageFont.load_default()
    draw = ImageDraw.Draw(canvas)
    panels = [
        ("Original generated image", source),
    ...
    for index, (label, image) in enumerate(panels):
        ...
        panel = Image.fromarray(to_uint8(image), mode="RGB").resize((tile_w, tile_h - 26), Image.Resampling.LANCZOS)
        canvas.paste(panel, (x, y + 26))
        draw_label(draw, (x + 12, y + 4), label, font)
...
def main() -> None:
    ...
    save_rgb("01_generated_scene.png", source)
    save_grid("02_convolution_filters.png", convolution_filter_grid(source), columns=2)
    save_grid("03_frequency_filtering.png", frequency_filtering(source), columns=2)
    save_grid("04_edge_detection_pipeline.png", edge_pipeline(source), columns=2)
    save_grid("05_object_detection_morphology.png", segmentation_morphology(source), columns=2)
    save_grid("06_opening_closing_challenge.png", opening_closing_challenge(), columns=2)
    save_rgb("07_naturalness_features.png", naturalness_features(source))
    print("Generated 7 project images in", OUT_DIR)
